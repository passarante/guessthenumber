...
export default function Home() {
  ...
  useEffect(() => {
    if (id) {
      getGameData(id.toString())
        .then((res) => {
          if (res && res.data) {
            setGame(res.data);
            setGuess(res.data.guess_number);
            const userGuesses = res.data.game_plays.map((g) => g.guess);
            setUserGuess(userGuesses);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  const checkGuess = () => {
    if (
      ...
      const currentGuess = parseInt(`${number1}${number2}${number3}${number4}`);
      setUserGuess([...userGuess, currentGuess]);
